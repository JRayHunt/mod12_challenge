...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    // use  slice(), map(), and reverse() from 12.2.1

    var topTenOtu_ids = otu_ids.slice(0,10);
    var yticks = topTenOtu_ids.map(otuID => `OTU ${otuID}`).reverse();
    ...
    d3.json("samples.json").then((data) => {
      ...
      // var wFreq = mdResult.wfreq.toFixed(1);
      var wFreq = parseFloat(mdResult.wfreq);
